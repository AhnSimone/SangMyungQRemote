	...
	private void run() throws IOException, WriterException {
		...
		while (true) {
			...
			if (line.startsWith("START")) {
				...
				System.out.println(line.substring(5));// 넘겨 받은 값 확인

				out.println(line.substring(5)); // 넘겨 받은 값을 확인 차 넘겨준다.
			...
			// System.out.println("qr 데이터를 받아 옵니다.");
			// out.println(test_qr);
			// } else if (line.startsWith("CONNECT")) {
			// System.out.println("모바일 과의 연결이 완료");
			else if (line.startsWith("CMDTEST")) {
				System.out.println("명령을 받습니다.");
				System.out.println(line.substring(8, 13)); // mouse를 짜름
...
//							+ line.substring(line.indexOf('h') + 1));
	...
	private void fire_event(int x, int y, int w, int h) {
		...
		mouseGlide(pointerInfo.getLocation().x, pointerInfo.getLocation().y,
				move_x, move_y, 10, 5); // 0.005 초로 1단위로 움직인다.
